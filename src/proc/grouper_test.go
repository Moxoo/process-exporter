		...
		},
		{
			...
			GroupByName{
				"g1": Group{Counts{1, 1, 1, 1, 1, 1, 0, 0}, States{Zombie: 1}, msi{}, 1,
					Memory{6, 7, 0, 0, 0}, starttime, 100, 0.25},
				"g2": Group{Counts{2, 2, 2, 2, 2, 2, 0, 0}, States{Running: 1}, msi{}, 1,
					Memory{9, 8, 0, 0, 0}, starttime, 400, 1},
			},
		},
	}

	gr := NewGrouper(newNamer(n1, n2), false)
...
func TestGrouperProcJoin(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			GroupByName{
				"g1": Group{Counts{}, States{}, msi{}, 1, Memory{3, 4, 0, 0, 0}, starttime, 4, 0.01},
			},
		}, {
			...
			GroupByName{
				"g1": Group{Counts{2, 2, 2, 2, 2, 2, 0, 0}, States{Running: 1, Sleeping: 1}, msi{}, 2,
					Memory{4, 6, 0, 0, 0}, starttime, 44, 0.1},
			},
		}, {
			[]IDInfo{
			...
			GroupByName{
				"g1": Group{Counts{4, 4, 4, 4, 4, 4, 0, 0}, States{Running: 2}, msi{}, 2,
					Memory{3, 9, 0, 0, 0}, starttime, 44, 0.1},
			},
		},
	}

	gr := NewGrouper(newNamer(n1), false)
	for i, tc := range tests {
		got := rungroup(t, gr, procInfoIter(tc.procs...))
		if diff := cmp.Diff(got, tc.want); diff != "" {
			t.Errorf("%d: curgroups differs: (-got +want)\n%s", i, diff)
...
func TestGrouperNonDecreasing(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			GroupByName{
				"g1": Group{Counts{}, States{}, msi{}, 2, Memory{4, 6, 0, 0, 0}, starttime, 44, 0.1},
			},
		}, {
			[]IDInfo{
			...
			GroupByName{
				"g1": Group{Counts{1, 1, 1, 1, 1, 1, 0, 0}, States{}, msi{}, 1, Memory{1, 5, 0, 0, 0}, starttime, 4, 0.01},
			},
		}, {
			[]IDInfo{},
			GroupByName{
				"g1": Group{Counts{1, 1, 1, 1, 1, 1, 0, 0}, States{}, nil, 0, Memory{}, time.Time{}, 0, 0},
			},
		},
	}
...
func TestGrouperThreads(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			GroupByName{
				"g1": Group{Counts{}, States{}, msi{}, 1, Memory{}, tm, 1, 1},
			},
		}, {
			piinfot(p, n, Counts{}, Memory{}, Filedesc{1, 1}, []Thread{
	...
	gr := NewGrouper(newNamer(n), false)
	for i, tc := range tests {
